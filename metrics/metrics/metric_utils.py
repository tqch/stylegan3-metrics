...
# noinspection
class MetricOptions:
    def __init__(self, dataset_kwargs=None, samp_kwargs=None, num_gpus=1, rank=0, device=None, progress=None,
                 cache=True):
        if dataset_kwargs is None:
            dataset_kwargs = {}
        if samp_kwargs is None:
            samp_kwargs = {}
        assert 0 <= rank < num_gpus
        self.dataset_kwargs = dnnlib.EasyDict(dataset_kwargs)
        self.samp_kwargs = dnnlib.EasyDict(samp_kwargs)
...
def compute_feature_stats_for_samp(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64,
                                   data_loader_kwargs=None, max_items=None, **stats_kwargs):
    dataset = dnnlib.util.construct_class_by_name(**opts.samp_kwargs)
    if data_loader_kwargs is None:
        data_loader_kwargs = dict(pin_memory=True, num_workers=3, prefetch_factor=2)
